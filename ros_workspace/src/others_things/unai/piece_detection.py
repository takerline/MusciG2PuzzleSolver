...
class PieceDetection:
    ...
    def get_piece_contours(self, img):
        ...
        alto = np.array([190, 145, 145]) # antes 180

        # Crear máscara basada en los umbrales.
        mascara_prueba = cv2.inRange(blur, bajo, alto)
        ...
        mask_canny = cv2.Canny(img_gray, 50, 140)
        ...
        black_background = np.zeros_like(mask_canny)

# Dibujar los contornos en el fondo negro
        cv2.drawContours(black_background, pieces_contours, -1, (255, 255, 0), 2)  # Color verde y grosor 2
        cv2.imshow('Contornos de las piezas', black_background)
    ...
    def PolygonDetection(self, pieces_contours) -> dict:
        ...
        for contorno in pieces_contours:
            ...
            aproximacion = cv2.approxPolyDP(contorno, 0.067 * perimetro, True) # 0.064
            print(f'vertices: {len(aproximacion)}')
...
#
    ...
    def process_image(self, img):
        ...
        cv2.imshow('Imagen final', img_corrected)
