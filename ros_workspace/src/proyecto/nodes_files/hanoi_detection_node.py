...
class HanoiNode():
    def __init__(self, name_node: str = "nodo_global", topic_orders: str = "hanoi_orders", 
        ...
        rospy.Subscriber(
            '/cam1/usb_cam_estiven_ASUS_TUF_Gaming_A15_FA506ICB_FA506ICB_383628_6738684236839502965/image_raw',
            Image,
            self.__cb_image
        )
        ...
        while not rospy.is_shutdown():
            ...
            if self.order_data is True:
                ...
                if len(self.group_images) == 10:
                    ...
                    game = HanoiGame(towers=3, discs=5,target_tower=2,start_tower=0,aux_tower=1,standard_start=False)
                    game.towers.append(HanoiTower(discs=most_votated[0]))
                    game.towers.append(HanoiTower(discs=most_votated[1]))
                    game.towers.append(HanoiTower(discs=most_votated[2]))
                    game.paintHanoi()
